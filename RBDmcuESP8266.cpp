...
void dimmerLamp::ext_int_init(void) 
{
	...
	attachInterrupt(inPin, isr_ext, FALLING); //was FALLING
}
 ...
 void ICACHE_RAM_ATTR isr_ext()
{
	for (int i = 0; i < current_dim; i++ ) 
      ...
      //digitalWrite(D3, !digitalRead(D3));  //debug
...
{	
	...
	for (k = 0; k < current_dim; k++)
	{
        ...
        //digitalWrite(D3, HIGH);	//debug
			}

			if (dimCounter[k] >=  (dimPulseBegin[k]-2 + pulseWidth) )
			{
				digitalWrite(dimOutPin[k], LOW);
        //check dimCounter value? to check if we are going to reach next zerocross
				zeroCross[k] = 0;
				dimCounter[k] = 0;
			}
		}
